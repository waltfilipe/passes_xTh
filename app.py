...
from external_models import get_v33_bonus_markov_key, load_markov_model
...
# ── PAGE CONFIG ────────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="Brasileirão 2026 — Passes xT")
...
@st.cache_data(show_spinner=False)
def load_team_match_counts(_cache_version: int = DATA_CACHE_VERSION) -> dict[str, int]:
    """Number of matches per team in the Brasileirão pass dataset."""
    if not SEASON_ALL_CSV_PATH.exists():
        return {}
    frame = pd.read_csv(
        SEASON_ALL_CSV_PATH,
        usecols=["event_id", "home_team", "away_team"],
        low_memory=False,
    )
    if frame.empty:
        return {}
    home = frame.groupby("home_team", sort=False)["event_id"].nunique()
    away = frame.groupby("away_team", sort=False)["event_id"].nunique()
    counts = home.add(away, fill_value=0).astype(int)
    return counts.to_dict()


@st.cache_data(show_spinner=False)
def load_player_minutes_info(_cache_version: int = DATA_CACHE_VERSION) -> dict[str, dict]:
    """Minutes played and team for each player (SofaScore box score)."""
    if not PLAYER_MATCH_STATS_PATH.exists():
        return {}
    ...
    stats["player_id"] = stats["player_id"].astype(str)
    stats["minutes_played"] = pd.to_numeric(
        stats.get("minutes_played", 0), errors="coerce"
    ).fillna(0.0)
    ...
    team_matches = load_team_match_counts(_cache_version)
    out: dict[str, dict] = {}
    for pid, grp in stats.groupby("player_id", sort=False):
        minutes = float(grp["minutes_played"].sum())
        team = grp["team"].mode().iloc[0] if not grp["team"].mode().empty else grp["team"].iloc[0]
        team = str(team)
        max_minutes = float(team_matches.get(team, 0) * 90)
        pct = (minutes / max_minutes) if max_minutes > 0 else 0.0
        out[str(pid)] = {
            "minutes": int(round(minutes)),
            "team": team,
            "max_minutes": int(max_minutes),
            "minutes_pct": round(pct, 4),
            "eligible_ranking": pct >= MIN_MINUTES_PCT,
        }
    return out
...
def _merge_box_stats(
    ...
    """Overlay SofaScore minutes onto pass-derived stats."""
    merged = dict(stats)
    extra = box_stats.get(player_code, {})

    for key in ("minutes", "pass_value"):
        if key in extra:
            val = extra[key]
            if key == "pass_value":
                merged[key] = round(float(val), 3)
            else:
                merged[key] = val
...
def _pass_player_metrics(
    ...
    """Build pass-only metrics for one player under heuristic v4."""
...
def _build_ranking_players(
    ...
    *,
    box_stats: dict[str, dict] | None = None,
    minutes_info: dict[str, dict] | None = None,
    min_minutes_pct: float = MIN_MINUTES_PCT,
) -> list[dict]:
    players: list[dict] = []
    for player in players_registry:
        if not is_outfield_position(player.get("position")):
            continue
        metrics = _pass_player_metrics(
            player_data, player, box_stats, minutes_info
        )
        if metrics is None:
            continue
        pct = metrics.get("minutes_pct")
        if pct is None or pct < min_minutes_pct:
            continue
...
def _ranking_table(
    ...
    *,
    ...
    ranked = sorted(players, key=lambda p: p.get(metric_key, 0) or 0, reverse=True)[:top_n]
    rows = []
        ...
        rows.append(
            {
                ...
                metric_label: fmt_fn(player.get(metric_key)),
...
def render_ranking_tab(
    ...
    *,
    ...
    if not players:
        st.warning("Nenhum jogador elegível para o ranking com o filtro de minutos.")
        return
...
if not players_registry or not any(not df.empty for df in player_data.values()):
    st.error(
        "Dataset de passes não encontrado ou vazio. "
