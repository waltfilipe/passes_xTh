...
Exemplo (Série A / B ou outra competição)::
    ...
    # Copiar consolidado para a raiz do app (season_all_serieb.csv)
    python -u scripts/onlypasses.py ... --copy-season-to-root --season-filename season_all_serieb.csv
"""
...
def fetch_match_passes(
    ...
    *,
    ...
    """Passes de uma partida: lineups + rating-breakdown por jogador."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Baixa passes com coordenadas do SofaScore (poucas chamadas à API)."
    )
    ...
    parser.add_argument("--resume", action="store_true", help="Pular jogos já em done_passes.json")
    parser.add_argument(
        "--consolidated-only",
        action="store_true",
        help="Acrescentar cada jogo direto em season_all.csv (sem CSV por partida)",
    )
    parser.add_argument(
        "--consolidate",
        action="store_true",
        help="Ao final, mesclar match_*_passes.csv em season_all.csv",
    )
    parser.add_argument(
        "--copy-season-to-root",
        action="store_true",
        help="Copiar season_all.csv consolidado para a raiz do repositório",
    )
    parser.add_argument(
        "--season-filename",
        default="season_all.csv",
        help="Nome do CSV consolidado (padrão: season_all.csv)",
    )
    ...
    for i, summary in enumerate(matches, start=1):
        ...
        try:
            ...
            _log(
                f"  ✓ FIM · {len(rows)} passes · "
                f"{with_passes}/{fetched} jogadores com dados"
            )
            if len(rows) == 0:
                _log(
                    "  AVISO: rating-breakdown sem passes — SofaScore pode não expor "
    ...
    if args.consolidated_only and season_path.exists():
        _log(f"Temporada: {season_path} ({total_pass_rows:,} passes nesta execução)")
